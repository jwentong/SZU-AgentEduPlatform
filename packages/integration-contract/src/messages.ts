...
export type HostToTeacherMessage =
  | IntegrationEnvelope<'HOST_CONTEXT', {
      tenantId: string;
      userId: string;
      courseId: string;
      scope?: CourseScope;
  ...
  | IntegrationEnvelope<'HOST_SCOPE_CHANGED', { courseId: string; scope: CourseScope }>
...
export type TeacherToHostMessage =
  | IntegrationEnvelope<'TEACHER_WORKSPACE_READY', {
      capabilities: string[];
      courseId?: string;
    }>
