...
### reveal (Animated Element Entrance)

```json
{ "type": "action", "name": "reveal", "params": { "elementId": "text_conclusion", "effect": "fade-up", "duration": 520 } }
```

- Use only for a concise conclusion, answer, synthesis, or key takeaway that should initially be hidden.
- Place `reveal` after the supporting content has been explained, then immediately follow it with speech explaining the revealed element.
- `elementId` must be an existing non-decorative element and should normally be used by at most one reveal action.
...
- When the slide contains a distinct conclusion/summary element, prefer one reveal+text pair near the end.
