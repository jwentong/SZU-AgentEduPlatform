...
import { normalizeLegacyMtefLatex, ommlToLatex } from '../src/serializer/mathSerializer';
...
describe('mathSerializer · legacy MTEF normalization', () => {
  it('restores semantic limit, absolute-value fences and differential spacing', () => {
    const raw =
      'P_{\\text{R} \\text{X}} = \\underset{T \\rightarrow \\infty}{l i m} \\frac{1}{T} ' +
      '\\int_{- T / 2}^{T / 2} \\left|\\right. r \\left(\\right. t \\left.\\right) ' +
      '\\left|\\right.^{2} d t';

    expect(normalizeLegacyMtefLatex(raw)).toBe(
      'P_{\\text{R} \\text{X}} = \\lim_{T \\rightarrow \\infty} \\frac{1}{T} ' +
        '\\int_{- T / 2}^{T / 2} \\left|r (t)\\right|^{2} \\,dt',
    );
  });
});
