 ...
 * - WMF → PNG through the host application's server-side converter
...
async function blobToDataUrl(blob: Blob): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = () => reject(reader.error || new Error('Failed to read converted image'));
    reader.readAsDataURL(blob);
  });
}

/**
 * Browsers cannot decode WMF. PowerPoint uses it as the preview for many
 * legacy MathType / Equation Editor OLE objects, so silently replacing it
 * with a transparent pixel removes the equation. Delegate that one format to
 * the host server, which rasterizes it in an isolated temporary directory.
 */
async function wmfToPngDataUrl(data: Uint8Array): Promise<string> {
  try {
    const requestBody = data.slice().buffer as ArrayBuffer;
    const response = await fetch('/api/convert-vector-image?format=wmf', {
      method: 'POST',
      headers: { 'content-type': 'application/x-msmetafile' },
      body: requestBody,
    });
    if (!response.ok) throw new Error(`WMF conversion failed (${response.status})`);
    return await blobToDataUrl(await response.blob());
  } catch (error) {
    console.error('[wmfToPng] failed:', error);
...
export async function resolveMediaToUrl(
  ...
  // A blob URL does not make a non-web image decodable. Convert legacy
  // formats first even when the caller otherwise prefers blob URLs.
  if (mode === 'blob' && !isNonWebFormat(mediaPath)) {
