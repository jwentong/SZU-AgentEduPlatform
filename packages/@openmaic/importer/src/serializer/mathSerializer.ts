...
/** Repair structural artifacts produced by mathml-to-latex for legacy MTEF. */
export function normalizeLegacyMtefLatex(latex: string): string {
  return latex
    // MTEF encodes “lim” as three identifier records under an underscript.
    .replace(/\\underset\{([^{}]+)\}\{l\s+i\s+m\}/g, '\\lim_{$1}')
    .replace(
      /\\left\|\\right\.\s*(.*?)\s*\\left\|\\right\./g,
      (_match, body: string) => `\\left|${body.trim()}\\right|`,
    )
    // The converter emits invisible delimiter pairs around ordinary fences.
...
async function resolveOleMathTypeLatex(rId: string, ctx: RenderContext): Promise<string> {
  const rel = ctx.slide.rels.get(rId);
  if (!rel) return '';
  const fileName = rel.target.split('/').pop() || '';
  const data = ctx.presentation.embeddings.get(`ppt/embeddings/${fileName}`);
  if (!data) return '';

  try {
    const response = await fetch('/api/convert-mathtype-formula', {
      method: 'POST',
      headers: { 'content-type': 'application/vnd.ms-office' },
      body: data.slice().buffer as ArrayBuffer,
    });
    if (!response.ok) return '';
    const result = (await response.json()) as { mathml?: unknown };
    if (typeof result.mathml !== 'string') return '';
    return normalizeLegacyMtefLatex(postProcessLatex(MathMLToLaTeX.convert(result.mathml)));
  } catch (error) {
    console.warn('[mathSerializer] MathType OLE→LaTeX conversion failed:', error);
    return '';
  }
}
...
export async function mathToElement(
  ...
  if (node.oleMathTypeRId) {
    latex = await resolveOleMathTypeLatex(node.oleMathTypeRId, ctx);
