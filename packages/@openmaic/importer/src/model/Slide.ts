...
export function parseChildNode(
  ...
  switch (tag) {
    ...
    case 'graphicFrame':
      ...
      // Legacy MathType / Equation Editor OLE → editable MTEF math node.
      {
        const oleMathType = parseOleMathTypeNode(child);
        if (oleMathType) return oleMathType;
      }
