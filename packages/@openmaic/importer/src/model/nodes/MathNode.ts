...
export interface MathNodeData extends BaseNodeData {
  ...
  oleDocxRId?: string;
  /** rId of a legacy MathType / Equation Editor OLE object containing MTEF. */
  oleMathTypeRId?: string;
}
...
/** Parse an Equation.DSMT4 or Equation.3 OLE frame as semantic math. */
export function parseOleMathTypeNode(graphicFrame: SafeXmlNode): MathNodeData | undefined {
  const base = parseBaseProps(graphicFrame);
  const graphicData = graphicFrame.child('graphic').child('graphicData');
  const altContent = graphicData.child('AlternateContent');
  if (!altContent.exists()) return undefined;
  ...
  if (!/^Equation\.(?:DSMT\d+|3)$/i.test(progId)) return undefined;
  const oleMathTypeRId = oleObj.attr('r:id') ?? oleObj.attr('id');
  if (!oleMathTypeRId) return undefined;

  let fallbackBlipEmbed: string | undefined;
  for (const branch of ['Fallback', 'Choice'] as const) {
    const branchOle = altContent.child(branch).child('oleObj');
    const pic = branchOle.exists() ? branchOle.child('pic') : altContent.child(branch).child('pic');
    if (!pic.exists()) continue;
    const blip = pic.child('blipFill').child('blip');
    fallbackBlipEmbed = blip.attr('embed') ?? blip.attr('r:embed');
    if (fallbackBlipEmbed) break;
  }
