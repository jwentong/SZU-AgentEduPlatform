...
describe('speech request hygiene', () => {
  it('rejects punctuation-only cues but keeps normal Chinese narration', () => {
    expect(isSpeakableSpeechText(' 。\n')).toBe(false);
    expect(isSpeakableSpeechText('无线信道')).toBe(true);
  });

  it('drops only unspeakable speech and preserves visual actions in order', () => {
    const visual = { id: 'spot', type: 'spotlight' } as Action;
    const actions = [
      { id: 'bad', type: 'speech', text: '。 ' } as SpeechAction,
      visual,
      { id: 'good', type: 'speech', text: '比较两类信道。' } as SpeechAction,
    ];
    expect(filterUnspeakableSpeechActions(actions)).toEqual([visual, actions[2]]);
  });
});
