...
`;
  ...
  describe('Qwen Image credential fallback', () => {
    it('reuses the managed DashScope Qwen TTS credential when no image-specific key exists', async () => {
      vi.stubEnv('TTS_QWEN_API_KEY', 'dashscope-shared-key');
      vi.stubEnv('TTS_QWEN_BASE_URL', 'https://dashscope.aliyuncs.com');
      const { getServerImageProviders, resolveImageApiKey, resolveImageBaseUrl } =
        await import('@/lib/server/provider-config');

      expect(getServerImageProviders()['qwen-image']?.models).toEqual(['qwen-image-max']);
      expect(resolveImageApiKey('qwen-image')).toBe('dashscope-shared-key');
      expect(resolveImageBaseUrl('qwen-image')).toBe('https://dashscope.aliyuncs.com');
    });
  });
