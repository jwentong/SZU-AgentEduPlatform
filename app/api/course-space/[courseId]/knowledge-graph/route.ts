...
import { isCourseDatabaseConfigured, readCourseKnowledgeGraphFromDatabase, saveCourseKnowledgeGraphToDatabase } from '@/lib/server/course-space-database';
...
export async function GET(_request: NextRequest, context: { params: Promise<{ courseId: string }> }) {
  ...
  const graph = await readCourseKnowledgeGraphFromDatabase(courseId);
  return apiSuccess({ graph: graph ?? null, databaseConfigured:isCourseDatabaseConfigured() });
}

export async function POST(request: NextRequest, context: { params: Promise<{ courseId: string }> }) {
  const { courseId } = await context.params;
  if (!isCourseDatabaseConfigured()) return apiError('INVALID_REQUEST', 503, '课程知识图谱需要配置 COURSE_DATABASE_URL');
