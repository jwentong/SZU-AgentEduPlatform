...
import {
  ...
  type CoursewareConversionMode,
  type CoursewareSourceKind,
...
// PPTX import is a direct high-fidelity path: imported canvas slides are kept
// intact and wrapped in the normal OpenMAIC Stage/Scene playback shell.
...
function HomePage({
  ...
  const [pendingCoursewareInstruction, setPendingCoursewareInstruction] = useState<string | null>(
    null,
  );
  ...
  const [pendingCoursewareAction, setPendingCoursewareAction] = useState<
    'pptx-import' | 'generate' | null
  >(null);
  const coursewareModeRef = useRef<CoursewareConversionMode | null>(null);
  ...
  const handlePptxImported = async (slides: Slide[], source: CourseSourceFingerprint) => {
    if (slides.length === 0) {
      toast.error('PPT 中没有可用的幻灯片');
      return;
    }

    const conversionMode = coursewareModeRef.current ?? 'faithful';
    let pptxImportStorageKey: string | undefined;
    try {
      const userProfile = useUserProfileStore.getState();
      // PPT-to-course conversion is explicitly a narrated classroom flow.
      // Respecting a stale browser preference here would create speech actions
      // without ever allocating audio assets in generation-preview.
      const audioSettings = useSettingsStore.getState();
      audioSettings.setTTSEnabled(true);
      // The normal default is browser-native-tts. PPT generation needs
      // server-side audio assets so the generated script can be replayed in
      // the classroom after the scene is persisted. Select the configured
      // Qwen provider explicitly instead of relying on a stale user setting.
      audioSettings.setTTSProvider('qwen-tts');
      const sessionId = nanoid();
      // Both quick-generation routes retain the source canvas. Faithful mode
      // uses it directly; Learning Skills uses it as the quality-preserving
      // fallback when an AI generation stage is unavailable.
      pptxImportStorageKey = `pptx-import-${sessionId}`;
      await storeImportedPptSlides(pptxImportStorageKey, slides, source);
      const pptContentDraft = buildImportedPptDraft(slides);
      const requirement =
        conversionMode === 'faithful'
          ? buildFaithfulCoursewareRequirement('pptx', slides.length)
          : buildLearningSkillsEnhancementRequirement('', 'pptx');
      const sessionState = {
        sessionId,
        requirements: {
          requirement,
        ...
        previewPhase: 'ppt-review' as const,
        sourceType: conversionMode === 'faithful' ? 'pptx-faithful' : 'learning-skills-enhanced',
        conversionMode,
        pptxImportStorageKey,
      ...
      if (embedded && courseId)
        window.parent.postMessage(
          { type: 'teacher-workbench-open-preview', courseId },
  ...
  } = useImportPptx({ onImported: handlePptxImported });
  ...
  const addCourseMaterials = (files: File[]) => {
    if (files.some(isCoursewareConversionFile)) {
      coursewareModeRef.current = null;
    }
    setForm((prev) => {
  ...
  const handleGenerate = async (
    modeOverride?: CoursewareConversionMode,
    ...
    const hasCoursewareMaterial = form.courseMaterials.some(isCoursewareConversionFile);
    const conversionMode = hasCoursewareMaterial
      ? (modeOverride ?? coursewareModeRef.current)
      : undefined;
    if (hasCoursewareMaterial && !conversionMode) {
      setPendingCoursewareAction('generate');
    ...
    try {
      const userProfile = useUserProfileStore.getState();
      const sourceKind: CoursewareSourceKind = form.courseMaterials.some((item) =>
        item.name.toLowerCase().endsWith('.pptx'),
      )
        ? 'pptx'
        : form.courseMaterials.some((item) => item.name.toLowerCase().endsWith('.pdf'))
          ? 'pdf'
          : 'document';
      const baseRequirement =
        instructionOverride?.trim() ||
        form.requirement.trim() ||
        '请根据教师上传的教学材料生成可自动播放的课堂课件。';
      ...
      let sourceFingerprint: CourseSourceFingerprint | undefined;
      ...
      if (form.courseMaterials.length > 0) {
        ...
        try {
          ...
          const primaryCourseware = orderedMaterials.find((item) =>
            isCoursewareConversionFile(item.file),
          );
          if (conversionMode && primaryCourseware) {
            sourceFingerprint = await fingerprintCoursewareFile(primaryCourseware.file, 0);
          }
      ...
      const sessionState = {
        ...
        sceneOutlines: null,
        currentStep: 'generating' as const,
        sourceType: conversionMode
          ? conversionMode === 'learning-skills-enhanced'
            ? ('learning-skills-enhanced' as const)
            : ('document-faithful' as const)
          : undefined,
  ...
  const openPptxConversionChoice = () => {
    setPendingCoursewareAction('pptx-import');
    setCoursewareModeOpen(true);
  };

  const handleCoursewareModeSelected = (mode: CoursewareConversionMode) => {
    coursewareModeRef.current = mode;
    const action = pendingCoursewareAction;
    setCoursewareModeOpen(false);
    setPendingCoursewareAction(null);
    if (action === 'pptx-import') {
      window.setTimeout(() => triggerPptxFileSelect(), 0);
      return;
    }
    if (action === 'generate') {
  ...
  const executeTeacherPlan = async (plan: TeacherOperationPlan) => {
    ...
    const response = await fetch(
      `/api/course-space/${encodeURIComponent(courseId)}/agent/operations`,
      {
        method: 'POST',
    ...
    if (data.dispatch === 'artifact-workflow') {
      ...
      const jobResponse = await fetch(`/api/course-space/${encodeURIComponent(courseId)}/jobs`, {
        ...
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          artifactTypes: [action.artifactType],
          scope: action.scope || commandScope(),
        }),
      });
      const jobData = await jobResponse.json();
      if (!jobResponse.ok || jobData.success === false)
        throw new Error(jobData.error || '创建教学产物任务失败');
      const jobId = jobData.jobs?.[0]?.id;
      window.parent.postMessage(
        jobId
          ? { type: 'teacher-workbench-open-job', courseId, jobId }
          : { type: 'teacher-workbench-course-updated', courseId },
        window.location.origin,
      );
  ...
  const sendTeacherCommand = async (command: string) => {
    ...
    if (/课件|幻灯片|\bPPT\b/iu.test(message)) {
      ...
      setTeacherCommandResult(
        `将根据当前工作目录和教师指令生成课件。确认后进入 OpenMAIC 课件生成与预览流程。\n教师指令：${message}`,
      );
      return;
    }
    if (
      /(?:生成|创建|编写|撰写).*(?:课程目标|课时目标|学习目标|知识点|教学活动)/u.test(message) &&
      !scope.startsWith('lesson:') &&
    ...
    try {
      const response = await fetch(`/api/course-space/${encodeURIComponent(courseId)}/agent`, {
        ...
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          sessionId: teacherSessionId.current,
          message,
          scope: commandScope(),
        }),
      });
      ...
      if (!response.ok || data.success === false)
        throw new Error(data.error || '备课智能体响应失败');
  ...
  return (
    ...
    >
      ...
      <CoursewareModeDialog
        ...
        onOpenChange={(open) => {
          setCoursewareModeOpen(open);
          if (!open) setPendingCoursewareAction(null);
        }}
        onSelect={handleCoursewareModeSelected}
        sourceLabel={pendingCoursewareAction === 'pptx-import' ? 'PPTX' : 'PDF / PPTX'}
      />
      {/* ═══ Top-right pill (unchanged) ═══ */}
      <div
        ref={toolbarRef}
        className={cn(
          'fixed top-4 right-4 z-50 flex items-center gap-1 bg-white/60 dark:bg-gray-800/60 backdrop-blur-md px-2 py-1.5 rounded-full border border-gray-100/50 dark:border-gray-700/50 shadow-sm',
          embedded && 'hidden',
        )}
      >
        {/* Language Selector */}
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div
          className="absolute top-0 left-1/4 h-96 w-96 rounded-full bg-[#B00055]/[0.07] blur-3xl animate-pulse"
          style={{ animationDuration: '4s' }}
        />
        <div
          className="absolute bottom-0 right-1/4 h-96 w-96 rounded-full bg-[#B00055]/10 blur-3xl animate-pulse"
      ...
      >
        ...
        {!embedded && (
          ...
          >
            <div className="inline-flex items-center gap-2 rounded-full border border-[#B00055]/20 bg-[#B00055]/5 px-4 py-2 text-sm font-medium text-[#B00055] dark:border-[#D34B87]/40 dark:bg-[#B00055]/15 dark:text-[#F08AB6]">
              <Presentation className="size-4" />
              快速生成课件
            </div>
            <Button
              variant="outline"
              className="rounded-full bg-white/70 px-4 dark:bg-slate-900/70"
        ...
        >
          <div className="w-full rounded-2xl border border-border/60 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl shadow-xl shadow-black/[0.03] dark:shadow-black/20 transition-shadow focus-within:border-[#B00055]/20 focus-within:shadow-2xl focus-within:shadow-[#B00055]/[0.07]">
            {embedded && (
              ...
              >
                <button
                  type="button"
                  disabled={teacherCommandBusy || !hasUsableProvider}
                  onClick={() =>
                    void sendTeacherCommand(
                      form.requirement.trim() ||
                        '请结合当前工作目录与课程材料生成一节 AI 增强课件。',
                    )
                  }
                  className="rounded-full border border-[#B00055]/20 bg-white/85 px-3 py-1.5 text-xs font-medium text-[#B00055] hover:bg-[#B00055]/10 disabled:opacity-50"
                >
                  课件生成
                </button>
                <button
                  ...
                  onClick={() =>
                    window.parent.postMessage(
                      { type: 'teacher-workbench-open-knowledge-graph', courseId },
                      window.location.origin,
                    )
                  }
                  className="rounded-full border border-[#B00055]/20 bg-white/85 px-3 py-1.5 text-xs font-medium text-[#B00055] hover:bg-[#B00055]/10"
                >
                  知识图谱
                </button>
              </div>
            ...
            {/* Textarea */}
            <div className="relative">
              {embedded && !form.requirement.trim() && (
                <div
                  aria-hidden="true"
                  className="pointer-events-none absolute left-4 top-4 space-y-1 text-[13px] leading-relaxed text-slate-400 dark:text-slate-500"
                >
                  <p>输入备课命令，例如：</p>
            ...
            <div className="px-3 pb-3 flex items-end gap-2">
              ...
              {PPTX_IMPORT_ENABLED && (
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      type="button"
                      onClick={openPptxConversionChoice}
                      disabled={pptxImporting}
                      className="inline-flex h-8 shrink-0 items-center gap-1.5 rounded-lg border border-[#B00055]/20 bg-[#B00055]/5 px-2.5 text-xs font-medium text-[#B00055] transition-colors hover:bg-[#B00055]/10 disabled:opacity-50"
                    >
              ...
              <button
                onClick={() =>
                  embedded ? void sendTeacherCommand(form.requirement) : void handleGenerate()
                }
          ...
          {embedded &&
            (teacherCommandResult || pendingTeacherPlan || pendingCoursewareInstruction) && (
              <div
                className="mt-3 rounded-xl border border-[#B00055]/15 bg-white/90 p-3 text-xs text-slate-700"
                role="status"
              >
                ...
                {pendingTeacherPlan && (
                  <div className="mt-2 flex gap-2">
                    <button
                      type="button"
                      disabled={teacherCommandBusy}
                      onClick={() => void confirmTeacherPlan()}
                      className="rounded-lg bg-[#B00055] px-3 py-1.5 font-medium text-white"
                    >
                      确认执行
                    </button>
                    <button
...
export default function Page() {
  ...
  const [view, setView] = useState<{
    mode: 'welcome' | 'generate' | 'embedded';
    courseId?: string;
    scope?: string;
    location?: string;
  }>({ mode: 'welcome' });
    ...
    <main className="flex min-h-screen items-center justify-center bg-[radial-gradient(circle_at_25%_15%,rgba(176,0,85,.13),transparent_35%),radial-gradient(circle_at_80%_75%,rgba(59,130,246,.09),transparent_38%),#fcf8fa] px-6 text-slate-900">
      <div className="w-full max-w-3xl rounded-[36px] border border-white/80 bg-white/75 px-8 py-16 text-center shadow-[0_35px_90px_-45px_rgba(98,23,59,.3)] backdrop-blur-xl sm:px-16">
        ...
        <p className="mx-auto mt-4 max-w-xl text-sm leading-7 text-slate-600">
          从课程进入，查看建设状态，并在 Class 工作页制作课件、整理教学产物和协同备课。
        </p>
