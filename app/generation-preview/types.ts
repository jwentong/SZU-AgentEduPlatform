...
export interface GenerationSessionState {
  ...
  previewPhase?: 'preparing' | 'ppt-review' | 'outline-ready' | 'review' | 'generating-content';
  pptContentDraft?: import('@/lib/types/generation').PptContentDraft;
  ...
  /** Conversion contract selected by the teacher before generation. */
  sourceType?:
    | 'pptx-structured-import'
    | 'pptx-faithful'
    | 'document-faithful'
    | 'learning-skills-enhanced';
  conversionMode?: import('@/lib/learning-skills/courseware-mode').CoursewareConversionMode;
  /** High-fidelity PPTX flow: source slides are stored in IndexedDB under this key. */
  pptxImportStorageKey?: string;
  /** Immutable source version used by traceability and release gates. */
  sourceFingerprint?: import('@/lib/course-governance/contracts').CourseSourceFingerprint;
...
export const getActiveSteps = (session: GenerationSessionState | null) => {
  return ALL_STEPS.filter((step) => {
    ...
    if (step.id === 'agent-generation') {
      // PPT conversion keeps the teacher roster stable and spends model calls
      // page-by-page on narration/actions instead of synthesizing extra agents.
      const isCoursewareConversion =
        Boolean(session?.conversionMode) || session?.sourceType === 'pptx-structured-import';
      return !isCoursewareConversion && useSettingsStore.getState().agentMode === 'auto';
    }
    return true;
  });
};
