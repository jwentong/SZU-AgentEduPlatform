...
export function RendererScreenCanvas() {
  ...
  const sourceSlide = useSceneSelector<SlideContent, SlideContent['canvas']>(
    (content) => content.canvas,
  );
  const slide = useMemo(() => withContentAwareViewport(sourceSlide), [sourceSlide]);
  ...
  const revealedElementIdList = useCanvasStore.use.revealedElementIdList();
  ...
  return (
    <SlideCanvas
      ...
      effects={effects}
      hiddenElementIds={hiddenElementIdList}
      entranceAnimatedElementIds={revealedElementIdList}
