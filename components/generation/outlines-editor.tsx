    ...
    >
        ...
        <div className="min-w-0 flex-1 space-y-1.5">
          ...
          {!isStreaming && (
            <p className="text-xs text-blue-600 dark:text-blue-300">
              点击每个场景的标题、说明或知识点即可直接修改，修改会自动保存。
            </p>
          )}
        </div>
        {onCollapse && (
      ...
      <div className="relative flex flex-col gap-3 border-t border-border/40 bg-gradient-to-t from-background/95 to-transparent px-6 py-4 md:flex-row md:items-center md:justify-between md:px-10 md:py-5">
        ...
        <div className="flex flex-col-reverse gap-2 md:flex-row md:items-center md:gap-2">
          ...
          >
            {backLabel ?? t('generation.backToRequirements')}
          </Button>
        ...
        <div className="min-w-0 flex-1 space-y-1.5">
          ...
          <div className="flex items-start justify-between gap-2">
            ...
            <textarea
              ...
              spellCheck={false}
              className={cn(
                'flex-1 resize-none rounded-md border border-transparent bg-transparent px-2 py-1 text-base font-semibold leading-7 tracking-tight transition-colors',
                'placeholder:font-normal placeholder:text-muted-foreground/40',
                'hover:border-blue-200 hover:bg-blue-50/50 focus:border-blue-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/15 dark:hover:border-blue-500/30 dark:hover:bg-blue-500/5 dark:focus:bg-slate-950 md:text-lg',
          ...
          <textarea
            ...
            rows={1}
            className={cn(
              'block w-full resize-none rounded-md border border-transparent bg-transparent px-2 py-1 text-sm leading-relaxed text-muted-foreground transition-colors',
              'placeholder:text-muted-foreground/40',
              'hover:border-blue-200 hover:bg-blue-50/50 focus:border-blue-400 focus:bg-white focus:text-foreground/90 focus:outline-none focus:ring-2 focus:ring-blue-500/15 dark:hover:border-blue-500/30 dark:hover:bg-blue-500/5 dark:focus:bg-slate-950',
              disabled && 'cursor-default',
            )}
          />
          ...
          <div className="flex flex-wrap items-center gap-1.5 pt-1">
            <AnimatePresence initial={false}>
              {(outline.keyPoints ?? []).map((point, idx) => (
                <EditableKeyPoint
                  key={`${outline.id}-kp-${idx}`}
                  value={point}
                  disabled={disabled}
                  onCommit={(value) => updateKeyPoint(idx, value)}
                  onRemove={() => removeKeyPoint(idx)}
                />
              ))}
            </AnimatePresence>
            {!disabled && (
...
function EditableKeyPoint({
  ...
  return (
    <motion.span
      ...
      className={cn(
        'group/chip inline-flex max-w-full items-center gap-1 rounded-full px-2.5 py-1 text-xs',
        'bg-muted/70 text-foreground/80 transition-colors',
        !disabled &&
          'hover:bg-blue-50 focus-within:bg-blue-50 focus-within:ring-2 focus-within:ring-blue-500/20 dark:hover:bg-blue-500/10 dark:focus-within:bg-blue-500/10',
      )}
    >
      <input
        value={draft}
        ...
        title={value}
        aria-label="编辑关键要点"
        size={Math.max(4, Math.min(36, draft.length + 1))}
        ...
        className="min-w-0 max-w-[28rem] border-none bg-transparent p-0 text-xs text-foreground/80 outline-none disabled:cursor-default"
      />
      {!disabled && (
        <button
          type="button"
