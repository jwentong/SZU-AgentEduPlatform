...
export function Stage({
  ...
  useEffect(() => {
    if (!forceEditMode && mode === 'edit' && !isEditable) {
      setMode('playback');
    }
  }, [forceEditMode, mode, isEditable, setMode]);
  ...
  useEffect(() => {
    if (forceEditMode && currentScene && mode !== 'edit') {
      void preloadEditor().then(() => setMode('edit'));
    }
  }, [currentScene, forceEditMode, mode, setMode]);
