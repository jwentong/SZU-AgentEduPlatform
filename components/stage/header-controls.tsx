...
import { CourseReleaseReviewDialog } from '@/components/governance/course-release-review-dialog';
import { evaluateCourseReleaseGate } from '@/lib/course-governance/governance';
import type { CourseReleaseGateResult } from '@/lib/course-governance/contracts';
...
export function HeaderControls({
  ...
  const [releaseGate, setReleaseGate] = useState<CourseReleaseGateResult>();
  ...
  const refreshReleaseGate = useCallback(async () => {
    if (!stage?.id) {
      setReleaseGate(undefined);
      return;
    }
    setReleaseGate(await evaluateCourseReleaseGate(stage.id, scenes));
  }, [stage?.id, scenes]);

  useEffect(() => {
    void refreshReleaseGate();
  }, [refreshReleaseGate]);
    ...
    <div className="flex items-center gap-4">
      ...
      {releaseGate?.applies && (
        <button
          ...
          title={releaseGate.ready ? '来源追踪与逐场景审核已通过' : '完成来源追踪与逐场景审核后才能发布'}
          className={cn(
      ...
      <CourseReleaseReviewDialog
        open={reviewDialogOpen}
        onOpenChange={setReviewDialogOpen}
        stageId={stage?.id}
        scenes={scenes}
        onChanged={refreshReleaseGate}
      />
    </div>
  );
}
