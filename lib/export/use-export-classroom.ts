...
export async function buildClassroomExportZip(
  ...
  try {
    ...
    const governance = await loadCourseGovernance(stage.id);
    if (governance) {
      zip.file(
        'governance.json',
        JSON.stringify(
          {
            ...governance,
            releaseGate: await evaluateCourseReleaseGate(stage.id, scenes),
          },
          null,
          2,
        ),
...
export function useExportClassroom() {
  ...
  const exportClassroomZip = useCallback(async () => {
    ...
    try {
      await assertCourseReleaseReady(stage.id, scenes);
