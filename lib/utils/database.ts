...
import type { CourseGovernanceRecord, CourseSourceFingerprint } from '@/lib/course-governance/contracts';
...
/**
 * Temporary high-fidelity PPTX import payload.
 *
 * Imported slides can contain large inline images, so they must not be placed in
 * sessionStorage. The record lives in IndexedDB until all scenes have been
 * materialized into the normal stage document.
 */
export interface PptxImportRecord {
  id: string;
  slides: Slide[];
...
class MAICDatabase extends Dexie {
  ...
  pptxImports!: EntityTable<PptxImportRecord, 'id'>;
  courseGovernance!: EntityTable<CourseGovernanceRecord, 'stageId'>;
  ...
  constructor() {
    ...
    // Version 18: temporary large-payload storage for high-fidelity PPTX imports.
    this.version(18).stores({
      pptxImports: 'id, createdAt',
    });

    // Version 19: mandatory source traceability, scene review and release gates
    // for PPT/PDF-derived AI education courses.
    this.version(19).stores({
      courseGovernance: 'stageId, source.sha256, updatedAt',
    });
  }
}
      ...
      withRuntimeStorageExclusiveLockUntilSettled(async (releaseCaller) => {
        ...
        await db.transaction(
          ...
          async () => {
            ...
            await db.courseGovernance.delete(stageId);
          },
        );
