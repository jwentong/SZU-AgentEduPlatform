...
function getApiHeaders(preferServerModel = false): HeadersInit {
...
export async function fetchSceneContent(
  ...
  try {
    return await withGenerationRetry(
      async () => {
        const { preferServerModel = false, ...requestParams } = params;
        const response = await fetch('/api/generate/scene-content', {
          method: 'POST',
          headers: getApiHeaders(preferServerModel),
          body: JSON.stringify(withThinkingConfig(requestParams, preferServerModel)),
          signal,
        });
...
export async function generateTTSForScene(
  ...
  return {
    // Preserve a usable page when one independent clip fails. Successful
    // clips remain attached; playback falls back to timed narration for the
    // missing cue. Only a total TTS failure blocks scene generation.
    success: failedCount === 0 || freshAllocations.length > 0,
    failedCount,
    error: lastError,
  };
}
...
export interface GenerationParams {
  ...
  userProfile?: string;
  languageDirective?: string;
  /** Original PPT canvases, indexed by fixed source-page order. */
  importedSlides?: Slide[];
  /** Reuse source canvases as content; false keeps them only for failure fallback. */
  preferImportedCanvas?: boolean;
  /** One-based scene orders whose source canvas is replaced by AI formula layout. */
  formulaEnhancedOrders?: number[];
  /** Audited, page-aligned narration; index follows fixed included-page order. */
  narrationScripts?: string[];
  /** Use the deployment's DEFAULT_MODEL instead of stale browser model settings. */
  preferServerModel?: boolean;
}
...
export function useSceneGenerator(options: UseSceneGeneratorOptions = {}) {
  ...
  const generateRemaining = useCallback(
    async (params: GenerationParams) => {
      ...
      const governanceEnabled = Boolean(await db.courseGovernance.get(stage.id));
      ...
      try {
        const applyAutomaticFallback = async (
          outline: SceneOutline,
          kind:
          ...
          const sourceSlide = params.importedSlides?.[outline.order - 1];
          const fallback = createAutomaticFallbackScene({
            stageId: stage.id,
            outline,
            sourceSlide,
            narrationScript: params.narrationScripts?.[outline.order - 1],
          });
          if (
            fallback.actions?.some((action) => action.type === 'speech') &&
            useSettingsStore.getState().ttsEnabled
          ) {
            await generateTTSForScene(fallback, params.languageDirective, signal).catch(() => ({
              success: false,
              failedCount: 1,
            }));
          }
          removeGeneratingOutline(outline.id);
          addGeneratedScene(fallback);
          await recordAutomaticFallback({
            stageId: stage.id,
            sceneOrder: outline.order,
            kind,
            reason,
            fallback: sourceSlide ? 'source-canvas' : 'text-slide',
          });
          options.onSceneGenerated?.(fallback, outline.order);
        };

        const fetchContent = (outline: SceneOutline): Promise<SceneContentResult> => {
          const importedSlide = params.importedSlides?.[outline.order - 1];
          const formulaEnhanced = params.formulaEnhancedOrders?.includes(outline.order) === true;
          if (params.preferImportedCanvas && importedSlide && !formulaEnhanced) {
        ...
        for (const outline of pending) {
          ...
          if (!contentResult.success || !contentResult.content) {
            ...
            const reason = contentResult.error || 'Content generation failed';
            if (governanceEnabled && params.preferImportedCanvas) {
              await applyAutomaticFallback(outline, 'content-generation-failure', reason);
              previousSpeeches = [];
              continue;
            }
            store.getState().addFailedOutline(outline);
            options.onSceneFailed?.(outline, reason);
            if (contentPromises) {
          ...
          if (actionsResult.success && actionsResult.scene) {
            ...
            if (
              ...
              if (!ttsResult.success) {
                ...
                const reason = ttsResult.error || 'TTS generation failed';
                if (governanceEnabled) {
                  await removeFreshTtsAllocations(speechAllocationIds(scene));
                  await recordAutomaticFallback({
                    stageId: stage.id,
                    sceneOrder: outline.order,
                    kind: 'tts-generation-failure',
                    reason,
                    fallback: params.importedSlides?.[outline.order - 1]
                      ? 'source-canvas'
                      : 'text-slide',
                  });
                } else {
            ...
            const reason = actionsResult.error || 'Actions generation failed';
            if (governanceEnabled && params.preferImportedCanvas) {
              await applyAutomaticFallback(outline, 'action-generation-failure', reason);
              previousSpeeches = [];
              continue;
            }
            store.getState().addFailedOutline(outline);
  ...
  const retrySingleOutline = useCallback(
    async (outlineId: string) => {
      ...
      const governanceEnabled = Boolean(await db.courseGovernance.get(state.stage.id));

      const applyRetryFallback = async (
        kind: 'content-generation-failure' | 'action-generation-failure' | 'tts-generation-failure',
        reason: string,
      ) => {
        const sourceSlide = params.importedSlides?.[outline.order - 1];
        const fallback = createAutomaticFallbackScene({
