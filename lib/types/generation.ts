...
export interface SceneOutline {
  ...
  order: number;
  /** Original zero-based PPT page index retained through outline review edits. */
  sourceSlideOriginalIndex?: number;
...
export interface ImportedPptSlideDraft {
  id: string;
  ...
  /** Semantic formulas extracted from OMML/MathType and stored as editable LaTeX. */
  formulas?: string[];
  /** Formula pages may be redrawn by OpenMAIC while retaining page identity/order. */
  generationStrategy?: 'faithful' | 'ai-formula-enhanced';
}

export interface PptContentDraft {
  sourceFileName?: string;
  /** Pins the routing/narration contract used to produce this reviewed draft. */
  generationPolicyVersion?: string;
  slides: ImportedPptSlideDraft[];
  revision: number;
  updatedAt: number;
}
