...
/** Qwen TTS and Qwen Image use the same DashScope account credential. */
function applyQwenImageFallback(
  imageConfig: Record<string, ServerProviderEntry>,
  yamlImageSection: Record<string, Partial<ServerProviderEntry>> | undefined,
): Record<string, ServerProviderEntry> {
  if (imageConfig[QWEN_IMAGE_PROVIDER_ID]) return imageConfig;
  const apiKey = process.env.TTS_QWEN_API_KEY || process.env.QWEN_API_KEY;
  if (!apiKey) return imageConfig;
  const yamlEntry = yamlImageSection?.[QWEN_IMAGE_PROVIDER_ID];
  imageConfig[QWEN_IMAGE_PROVIDER_ID] = {
    ...
    models: yamlEntry?.models || ['qwen-image-max'],
    proxy: yamlEntry?.proxy,
  };
  return imageConfig;
}
...
function buildConfig(yamlData: YamlData): ServerConfig {
  const image = applyQwenImageFallback(
...
/** Managed image providers pin their first server-approved model. */
