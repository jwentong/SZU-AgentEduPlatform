...
import { isSpeakableSpeechText } from '@/lib/audio/tts-utils';
...
export class PlaybackEngine {
  ...
  async jumpToAction(actionIndex: number, options: { autoplay?: boolean } = {}): Promise<boolean> {
    ...
    this.actionEngine.resetPlaybackVisualState(
      actions.filter((action) => action.type === 'reveal').map((action) => action.elementId),
    );

    for (let i = 0; i < actionIndex; i++) {
      if (!this.isCurrentGeneration(generation)) return false;
      const action = actions[i];
      if (isWhiteboardPlaybackAction(action) || action.type === 'reveal') {
  ...
  private async processNext(generation: number = this.playbackGeneration): Promise<void> {
    ...
    switch (action.type) {
      case 'speech': {
        ...
        if (!isSpeakableSpeechText(speechAction.text)) {
          this.processNext(generation);
          break;
        }
