...
/** True when text contains something a speech provider can pronounce. */
export function isSpeakableSpeechText(text: string | undefined | null): boolean {
  if (!text) return false;
  return /[\p{L}\p{N}]/u.test(text.normalize('NFKC'));
}

/** Remove provider-invalid speech cues without disturbing visual action order. */
export function filterUnspeakableSpeechActions(actions: Action[]): Action[] {
  let changed = false;
  const filtered = actions.filter((action) => {
    const keep = action.type !== 'speech' || isSpeakableSpeechText(action.text);
    if (!keep) changed = true;
    return keep;
  });
  return changed ? filtered : actions;
}
