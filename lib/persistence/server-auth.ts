...
function devAuthenticatorAllowedInCurrentEnvironment(): boolean {
  return process.env.NODE_ENV !== 'production' || insecureDevAuthOptInEnabled();
}

if (process.env.NODE_ENV === 'production' && insecureDevAuthOptInEnabled()) {
  log.warn(
    'Persistence is using development authentication in production. It provides no user isolation.',
  );
}
